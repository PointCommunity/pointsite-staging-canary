...
for (const version of [10, 11] as const)
  test(`schema ${version} publications retain required images and fonts and preserve source documents`, async () => {
    ...
    try {
      ...
      const navigation =
        document.navigationDesigns?.[0].items ?? document.navigation;
      navigation[0].href = "/assets/menu-download";
      assert.ok(
        publicationMediaPaths(document).includes("/assets/menu-download"),
      );
      document.media.push({
        id: crypto.randomUUID(),
      ...
      await writeFile(
        join(root, "out/assets/unused.png"),
        "preserve on failure",
      );
